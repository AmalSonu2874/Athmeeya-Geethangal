(function(){
  ...
  const STORAGE = {
    ...
    installSeen: 'athmeeya-install-prompt-seen-v1'
  };
  const state = {
    query: '',
    indexQuery: '',
  ...
  function matches(song,q){
    const n = normalize(q);
    if(!n) return true;
    const clean = n.replace(/^(ഗീതം|song)\s*/,'').trim();
    if(/^\d+$/.test(clean)) return String(song.num) === clean;
  ...
  function renderJumpGrid(query=''){
    ...
    const available = new Set(SONGS.map(song => song.num));
    const titles = new Map(SONGS.map(song=>[song.num,song.title]));
  ...
  function navigateHash(number, push=true){
    const hash = `#song-${number}`;
    try{
      if(push) history.pushState({song:Number(number)},'',hash);
      else history.replaceState({song:Number(number)},'',hash);
  ...
  function scrollToSongCard(card, options={}){
    ...
    // All song cards stay in normal document flow, so the browser has an exact
    // position for every song. scroll-margin-top reserves space for the sticky header.
    card.classList.add('song-navigation-target');
    void card.offsetTop;

    const behavior=options.instant||prefersReducedMotion?'auto':'smooth';
    card.scrollIntoView({behavior,block:'start',inline:'nearest'});

    card.classList.remove('focus-song');
    void card.offsetWidth;
    card.classList.add('focus-song');
    card.setAttribute('tabindex','-1');
    try{card.focus({preventScroll:true});}catch(_){}

    // A final correction after the smooth scroll begins handles mobile browser
    // address-bar/header resizing without producing a visible jump.
    if(!options.instant && !prefersReducedMotion){
      setTimeout(()=>{
        const target=card.getBoundingClientRect().top;
        const header=$('site-header');
        const headerHeight=header ? header.getBoundingClientRect().height : 0;
        const desired=headerHeight+12;
        const delta=target-desired;
  ...
  function openSongFromHash(){
    ...
    const match = hash.match(/^#(?:song-|song\/)(\d+)$/i);
    if(!match) return;
    const song = getSong(Number(match[1]));
    if(song) jumpToSong(song.num,{instant:true,updateHash:false});
  }

  function normalizeSearchQuery(query){
    return normalize(query).replace(/^(?:ഗീതം|song)\s*/,'').trim();
  }

  function resolveExactSong(query){
    const n = normalize(query);
    if(!n) return null;
    const numberPart = normalizeSearchQuery(query);
    if(/^\d+$/.test(numberPart)) return getSong(Number(numberPart));
    const exact = SONGS.find(song => normalize(song.title)===n || normalize(song.title)===numberPart);
  ...
  function rankedSearchResults(query){
    ...
    if(!clean) return [];
    const exactNumber=/^\d+$/.test(clean) ? Number(clean) : null;
    const exactSong=exactNumber ? getSong(exactNumber) : SONGS.find(song=>normalize(song.title)===n || normalize(song.title)===clean);
    ...
    for(const group of [titleStarts,titleIncludes,broad]){
      for(const song of group){
        if(!seen.has(song.num)){seen.add(song.num);out.push(song);}
      }
    }
    return out;
  }
  ...
  function handleClick(event){
    ...
    const suggest=event.target.closest('[data-suggest]');if(suggest){jumpToSong(Number(suggest.dataset.suggest));$('search-suggestions').hidden=true;return;}
  ...
  function init(){
    ...
    const splash=$('splash');
    if(splash)setTimeout(()=>splash.classList.add('leave'),prefersReducedMotion?250:900);
